...
CREATE OR REPLACE FUNCTION pipeline.trigger_register_event() RETURNS trigger AS $function$
  DECLARE
    insert_sql TEXT;
    event_status pipeline.VIOME_EVENT_STATUS := 'pending';
  ...
  BEGIN 
    IF NEW.meta_data ? 'meta_data' THEN
      ...
      IF event_data ? 'event_status' THEN
        event_status := CAST (event_data ->> 'event_status' AS pipeline.VIOME_EVENT_STATUS);
      END IF;
      IF event_data ? 'pipeline' THEN
        event_pipeline := event_data ->> 'pipeline';
      END IF;
      SELECT event_data ->> 'viome_event' AS "event_token", NEW.created AS "created", event_meta AS "meta_data", NEW.event_id AS "event_id", event_pipeline AS "pipeline", event_status AS "event_status", NEW.updated AS "updated" INTO event_record;
      RETURN event_record;
    ELSE 
      RETURN NEW;
    END IF;
