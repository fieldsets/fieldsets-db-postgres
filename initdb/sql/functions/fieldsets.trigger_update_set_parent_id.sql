/**
 * trigger_update_set_parent_id: triggered after insert. Set parent id using parent_token.
...
BEGIN
  IF NEW.parent IS NULL THEN
    SELECT s.id INTO parent_id FROM fieldsets.sets s WHERE s.token = NEW.parent_token;
    IF parent_id >= 0 THEN
