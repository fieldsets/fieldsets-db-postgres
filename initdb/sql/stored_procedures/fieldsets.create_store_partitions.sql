 ...
 **/
CREATE OR REPLACE PROCEDURE fieldsets.create_store_partitions(parent_partition TEXT = 'fieldsets', parent_partition_token TEXT = 'fieldset') AS $procedure$
DECLARE
    tbl_sql TEXT;
...
BEGIN
    ...
    LOOP
        ...
        tbl_sql := format('CREATE TABLE IF NOT EXISTS %s PARTITION OF fieldsets.%s FOR VALUES IN (%L) TABLESPACE fieldsets;', store_partition_table_name, parent_partition, store_type_name);
        EXECUTE tbl_sql;
