/**
 * attach_fieldsets_partition: Run as a one time cron job. Attach a partition and unschedule the job.
 * @param TEXT: partition_token
 * @param TEXT: parent_table_name
 * @depends FUNCTION: trigger_create_sets_partition
 **/
CREATE OR REPLACE PROCEDURE fieldsets.attach_fieldsets_partition(partition_token TEXT, parent_table_name TEXT) AS $procedure$
  DECLARE
    create_tbl_sql TEXT;
    cron_job_token TEXT;
  BEGIN 
...
COMMENT ON PROCEDURE fieldsets.attach_fieldsets_partition (TEXT,TEXT) IS 
'/**
 * attach_fieldsets_partition: Run as a one time cron job. Attach a partition and unschedule the job.
 * @param TEXT: partition_token
 * @param TEXT: parent_table_name
