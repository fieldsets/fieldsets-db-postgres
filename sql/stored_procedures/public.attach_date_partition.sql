 ...
 **/
CREATE OR REPLACE PROCEDURE public.attach_date_partition(db_schema TEXT, parent_table_name TEXT, partition_table_name TEXT, partition_year NUMERIC, partition_month NUMERIC) AS $procedure$
  DECLARE
    create_tbl_sql TEXT;
    cron_job_token TEXT;
    range_start TEXT;
...
COMMENT ON PROCEDURE public.attach_date_partition (TEXT,TEXT,TEXT,NUMERIC,NUMERIC) IS 
'/**
 * attach_date_partition: Run as a one time cron job. Attach a partition and unschedule the job.
 * @param TEXT: db_schema
 * @param TEXT: parent_table_name
