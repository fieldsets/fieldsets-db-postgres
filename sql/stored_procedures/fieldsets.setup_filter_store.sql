...
BEGIN
    ...
    -- Filters are columns within the same table for a defined set, so we utilize the set token here are the parent.
    fieldset_parent_token := fs_record.set_token;
    SELECT id, token, label, parent, parent_token, set_id, set_token, field_id, field_token, type, store
    FROM fieldsets.fieldsets INTO fs_parent WHERE token = fieldset_parent_token;
    ...
    SELECT public.in_array(fs_parent.id, partition_ids) INTO exist_check;
    IF NOT exist_check THEN
        SELECT array_prepend(fs_parent.id, partition_ids) INTO partition_ids;
    END IF;
    ...
    LOOP
        IF fs IS NOT NULL THEN
            ...
            IF fs.type::TEXT = 'fieldset'::TEXT THEN
                ...
                exist_check := public.constraint_exists('fieldsets', partition_name, key_name);
                IF NOT exist_check THEN
                    sql_stmt := format('ALTER TABLE fieldsets.%I ADD CONSTRAINT %I FOREIGN KEY (%s) REFERENCES fieldsets.fieldsets(id) DEFERRABLE;', partition_name, key_name, fs.token);
                    EXECUTE sql_stmt;
                END IF;
            END IF;
        END IF;
